...
exports.run = async (client, message, args) => {
 ...
 if (args.length < 1) return message.channel.send("Bir üye etiketle ve tekrardan dene!")
    ...
    let time = args[1];
    if(!time || !ms(time)) return message.react('❌')
...
sicil.push(`sicil.${user.id}`, {
    ...
    const voicemutelendi = new Discord.MessageEmbed()
            ...
            .setColor("#2F3136")
            .setDescription(`
           ${user ? user.toString() : user.username}  üyesi ${message.author} Tarafından ${yaziSure} boyunca Seste Mutelendi!
Ceza Puan: \`${cpuan}\`
Ceza ID: \`#${Cezaİd}\`
...
const voicemutekalktı = new Discord.MessageEmbed()
            ...
            .setColor("#2F3136")
            .setDescription(`
            ${user ? user.toString() : user.username} üyesi'nin Voice Mutesi Kalktı!
