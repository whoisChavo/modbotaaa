...
if(["957610112485314581","957610102834212954"].some(e => member.roles.cache.has(e))) return message.lineReply("Kullanıcı zaten kayıtlı.").then(sil => sil.delete({timeout: 5000}));
 
 await message.guild.members.cache.get(member.id).setNickname(`● ${name}`)
 db.push(`isimler_${member.id}`, ` \`● ${name}\` (<@&erkekrolid>) yetkili:${message.author}`);
 ...
 const Webhook1 = new Discord.WebhookClient("957639979817922590", "c9tolcAck-ubN11oA9UUH8ULb8TsPu5KKY1gEGy1L0A_sFyvZw_bO-vNtkLElIR2qmCy")
 const tadashi = new Discord.MessageEmbed()
.setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true }))
.setColor('#2F3136')
.setFooter('Rahatsız olmamanız için embed şeklinde gönderiyorum!')
.setDescription(`**${member} üyesi kayıt oldu! Sunucuya Hoşgeldin.**`)
    Webhook1.send(tadashi)
