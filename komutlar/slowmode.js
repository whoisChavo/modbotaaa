...
exports.run = async (client, message, args) => {
  ...
  let adoembed = new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.member.user.avatarURL({dynamic:true}))
  ...
  message.channel.send(adoembed.setDescription(`Slow Mode **${miktar ? miktar : "Kapalı" }** saniye Olarak Ayarlandı.`)).then(m => m.delete({timeout: 5000}))
